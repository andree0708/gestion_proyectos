  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-purple-50">
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-3">
              ...
              <Link to="/" className="text-2xl font-bold">SubPro</Link>
            </div>
            <div className="flex items-center gap-3">
              <Link to="/login" className="flex items-center gap-2 px-4 py-2 bg-white/20 rounded-full hover:bg-white/30 transition font-medium text-sm">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" />
                </svg>
                Iniciar Sesión
              </Link>
              <Link to="/register" className="flex items-center gap-2 px-4 py-2 bg-white text-rose-600 rounded-full hover:bg-rose-50 transition font-medium text-sm">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="text-center mb-4">
            <h1 className="text-3xl font-light mb-2">Subproductos Alimenticios</h1>
            <p className="text-white/70">Encuentra y compra subproductos de la industria alimentaria</p>
          </div>
          ...
          <form onSubmit={handleSearch} className="relative max-w-2xl mx-auto">
            ...
            <input
              type="text"
              placeholder="Buscar harinas, frutas, lácteos, aceites, residuos..."
              value={search}
              onChange={e => setSearch(e.target.value)}
              className="w-full pl-12 pr-32 py-4 text-lg rounded-full shadow-xl focus:outline-none focus:ring-4 focus:ring-rose-200 text-gray-800"
            />
            <button type="submit" className="absolute right-2 top-2 bottom-2 px-6 bg-gradient-to-r from-rose-500 to-pink-500 text-white rounded-full hover:from-rose-600 hover:to-pink-600 transition font-medium flex items-center gap-2">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
              Buscar
          ...
          <div className="mt-4 flex gap-2 flex-wrap justify-center">
            ...
            {['Harinas', 'Frutas', 'Lácteos', 'Aceites'].map(tag => (
              <button key={tag} onClick={() => setSearch(tag)} className="px-3 py-1 bg-white/10 rounded-full text-sm hover:bg-white/20 transition text-white/80">
                {tag}
              </button>
            ))}
          </div>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <Link 
          to="/dashboard" 
          className="flex items-center gap-2 bg-gradient-to-r from-rose-500 to-purple-500 text-white px-5 py-3 rounded-full shadow-lg hover:shadow-xl hover:scale-105 transition-all font-medium"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {listings.map(listing => {
                    ...
                    return (
                    <Link key={listing.id} to={`/catalog/${listing.id}`} className={`card hover:shadow-xl transition-all hover:-translate-y-1 group bg-white rounded-xl overflow-hidden ${isOutOfStock ? 'opacity-80' : ''}`}>
                      <div className="aspect-video bg-gradient-to-br from-gray-100 to-gray-200 relative overflow-hidden">
                        {listing.photos?.[0] ? (
                          <img src={uploadsSrc(listing.photos[0])} alt={listing.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                        ) : (
                      ...
                      <div className="p-4">
                        ...
                        <h3 className="font-semibold text-gray-800 group-hover:text-rose-500 transition-colors line-clamp-2 mb-1 font-medium">
                          {listing.title}
                        </h3>
                        {listing.logisticsType && (
                          <p className="text-xs text-rose-600 mb-2">{logisticsBadge(listing.logisticsType)} · {listing.originCity}, {listing.originDepartment}</p>
                        )}
                        ...
                        <div className="flex items-center justify-between text-sm mb-3">
                          ...
                          <span className={`text-xs font-medium ${isOutOfStock ? 'text-red-500' : 'text-emerald-600'}`}>
                            {isOutOfStock ? 'Sin stock' : `${listing.quantity} disponibles`}
                          </span>
                        </div>
